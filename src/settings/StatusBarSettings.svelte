...
<div>
	...
	<p>
		Here you can customize what statistics are displayed on the status bar
		when editing a markdown note.
	</p>
				...
				<span class="bwc-sb-buttons">
					{#if i !== 0}
						<button
							...
							on:click={async () => {
								statusItems = swapStatusBarItems(
									i,
									i - 1,
									statusItems,
								);
				...
				<div class="setting-item-control">
					<select
						...
						on:change={async (e) => {
							...
							item.metric.counter =
								MetricCounter[MetricCounter[value]];
							await update(statusItems);
							await plugin.saveSettings();
						}}
					>
						...
						<option value={MetricCounter.characters}
							>Characters</option
						>
						<option value={MetricCounter.sentences}
							>Sentences</option
						>
						<option value={MetricCounter.footnotes}
							>Footnotes</option
						>
						<option value={MetricCounter.citations}
							>Citations</option
						>
	...
	<p>
		Here you can customize what statistics are displayed on the status bar
		when not editing a markdown file.
	</p>
	<div class="bwc-sb-buttons">
		<button
			aria-label="Add New Status Bar Item"
